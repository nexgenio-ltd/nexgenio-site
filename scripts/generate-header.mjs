 ...
 *
 * Outputs branded PNGs in three formats:
 *   - OG/LinkedIn: 1200x630
...
const STYLE_CLAUSE = 'Minimal geometric line illustration, abstract and structural. Thin clean strokes. Navy, orange and green line work on pure white background. No text, no words, no letters, no numbers, no photorealism, no people, no stock-photo aesthetic.';
...
function buildOgHtml(title, kicker, illustrationDataUri) {
...
function buildInstaPortraitHtml(title, kicker, illustrationDataUri) {
...
.accent-top { position:absolute; top:0; left:0; right:0; height:6px; background:#f6911b; z-index:2; }
.accent-bottom { position:absolute; bottom:0; left:0; right:0; height:3px; background:#215675; z-index:2; }
.illustration { width:1080px; height:700px; overflow:hidden; }
.illustration img { width:100%; height:100%; object-fit:cover; }
.content { padding:48px 56px 40px; }
.kicker { font-size:14px; font-weight:700; letter-spacing:2.5px; text-transform:uppercase; color:#f6911b; margin-bottom:16px; }
.title { font-size:${ts.fontSize}; font-weight:bold; line-height:${ts.lineHeight}; color:#215675; }
.rule { width:72px; height:4px; background:#f6911b; margin-top:24px; border-radius:2px; }
.logo-area { position:absolute; bottom:36px; left:56px; display:flex; align-items:center; gap:14px; z-index:2; }
.logo-area img { height:36px; width:auto; }
.logo-area .url { font-size:14px; color:#999; }
</style></head><body>
<div class="accent-top"></div>
<div class="illustration"><img src="${illustrationDataUri}"></div>
<div class="content">
...
function buildInstaSquareHtml(title, kicker, illustrationDataUri) {
...
const FRAME_BUILDERS = {
  og: buildOgHtml,
  'insta-portrait': buildInstaPortraitHtml,
  'insta-square': buildInstaSquareHtml,
};
...
async function main() {
  ...
  for (const fmt of formatList) {
    ...
    const builder = FRAME_BUILDERS[fmt];
    const html = builder(title, kicker, illustrations[fmt]);
    const page = await browser.newPage({ viewport: { width: spec.w, height: spec.h } });
    await page.setContent(html, { waitUntil: 'load' });

    const outFile = `${outPrefix}-${fmt}.png`;
